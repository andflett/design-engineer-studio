...
| Framework plugin (SvelteKit) | ✅ SvelteKit plugin + .svelte template annotation (`@designtools/svelte-plugin`) |
| Scoped style writes | ✅ `.astro` and `.svelte` scoped `<style>` block CSS property writes (`scoped-style.ts`) |
| SFC class manipulation | ✅ replaceClass/addClass for `.astro` and `.svelte` files (regex-based) |
| postMessage protocol | Selection, inline style preview, token preview, revert |
| Test suite | ✅ 303 tests — unit, AST fixtures, server integration, theme resolution, Astro transform, Svelte transform, scoped styles (vitest + supertest) |
...
- **Status:** Done
- **What:** SvelteKit plugin wrapping the Vite plugin from 2a, with `.svelte` template annotation via `svelte/compiler`
- **Implementation:** `@designtools/svelte-plugin` — Vite `load` hook parses `.svelte` files with `svelte/compiler` (modern AST mode), walks fragment nodes recursively (including `{#if}`, `{#each}`, `{#await}`, `{#key}`, `{#snippet}` blocks), and splices `data-source`/`data-instance-source` attributes at AST offsets. Surface mounted via `transformIndexHtml` injecting React bootstrap script. Scoped `<style>` block writes handled by `scoped-style.ts`. SFC class manipulation (replaceClass/addClass) via `sfcReplaceOrAddClass()` regex helper in `write-element.ts`
...
| 2d. Svelte/SvelteKit | L | Small passionate niche | **P3** | ✅ Done |
...
6. ✅ **Svelte/SvelteKit plugin** (2d) — `@designtools/svelte-plugin` wrapping vite-plugin + `.svelte` template annotation via `svelte/compiler`
7. ✅ **Scoped style writes** — framework-agnostic `scoped-style.ts` for `.astro` and `.svelte` `<style>` block CSS property writes
8. ✅ **SFC class manipulation** — `sfcReplaceOrAddClass()` regex helper for replaceClass/addClass on `.astro` and `.svelte` files

The pitch is now real: "surface works on your existing project, whatever styling approach you use, whether it's Next.js, Vite, Astro, or SvelteKit."

9. ✅ **npm component handling** — editability tiers (full/instance-only/inspect-only), read-only mode for ComputedPropertyPanel, "go to parent instance" action, Page Explorer package badges
...
| Tier | What | Status |
|------|------|--------|
| Unit tests | Pure functions (tailwind-parser, tailwind-map, oklch, safe-path, write-css-rule, mount-transform, resolve-tailwind-theme, astro-source-transform, svelte-source-transform, scoped-style) | Done |
