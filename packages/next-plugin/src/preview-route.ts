...
export default function PreviewLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en" suppressHydrationWarning>
      <body style={{ margin: 0, padding: 32, background: "var(--background, #fff)", color: "var(--foreground, #000)", minHeight: "100vh" }}>
        {children}
      </body>
    </html>
  );
...
export default function PreviewPage() {
  ...
  if (error) {
    return (
      <div style={{ padding: 32, color: "var(--destructive, #ef4444)", fontFamily: "monospace", fontSize: 14 }}>
        {error}
      </div>
    );
  }
  ...
  if (!Component) {
    return (
      <div style={{ padding: 32, color: "var(--muted-foreground, #888)", fontFamily: "inherit", fontSize: 14 }}>
