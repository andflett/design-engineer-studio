...
/**
 * Spacing reverse lookup: maps computed px/rem values to Tailwind spacing scale numbers.
 * Tailwind spacing: 1 unit = 0.25rem = 4px (at default 16px base).
 */
...
/** Spacing properties that use the spacing scale. */
...
export interface TailwindMatch {
  ...
  /** Whether this is an exact scale match (tier 2) or arbitrary fallback (tier 3) */
  exact: boolean;
}

/**
 * Map a computed CSS value to a Tailwind class.
 * Returns the best Tailwind class for the given CSS property and computed value.
 */
export function computedToTailwindClass(
  cssProp: string,
  computedValue: string
): TailwindMatch | null {
  // Check direct reverse map first (keywords, font-size, font-weight, etc.)
  const directMap = REVERSE_MAP[cssProp];
  if (directMap?.[computedValue]) {
    return { tailwindClass: directMap[computedValue], exact: true };
  }

  // Spacing properties: check spacing scale
  ...
  if (RADIUS_PROPS.has(cssProp)) {
    ...
    if (radiusClass) {
      // For individual corners, adjust the class
      const prefix = CSS_TO_TW_PREFIX[cssProp];
      if (prefix) {
        // RADIUS_MAP gives generic "rounded-*", extract the size suffix
  ...
  // Generic arbitrary fallback for properties with a known prefix
  const prefix = CSS_TO_TW_PREFIX[cssProp];
  if (prefix) {
    return { tailwindClass: `${prefix}-[${computedValue}]`, exact: false };
  }
...
/**
 * For uniform box values (all 4 sides equal), generate a shorthand Tailwind class.
 * e.g., if all padding sides are 16px → "p-4" instead of "pt-4 pr-4 pb-4 pl-4"
 */
...
/**
 * For axis-symmetric box values, generate px/py or mx/my Tailwind classes.
 */
...
/**
 * For uniform border-radius, generate a single rounded-* class.
 */
...
/**
 * Try to match a computed color value to a token name from the scan data.
 * Returns the token reference (e.g., "primary") if found, null otherwise.
 */
export function matchColorToToken(
  computedColor: string,
  tokenGroups: Record<string, any[]>
): string | null {
  // Normalize the computed color for comparison
...
export interface TokenMatch {
  /** Token name without -- prefix, e.g. "spacing-4" or "radius-lg" */
  tokenName: string;
  /** Full CSS custom property name, e.g. "--spacing-4" */
  tokenVar: string;
  /** The token category */
  category: string;
  /** All tokens in the same group, for building a picker */
  groupTokens: Array<{ name: string; value: string }>;
}

/**
 * Try to match a computed value to ANY design token (not just colors).
 * Checks the token's resolved value against the computed value.
...
export function matchValueToToken(
  ...
  // For colors, use color-specific normalization
    ...
    for (const [, tokens] of Object.entries(tokenGroups)) {
      for (const t of tokens as any[]) {
        if (t.category === "color") {
          const tokenColor = normalizeColor(t.lightValue || "");
          if (tokenColor && tokenColor === normalized) {
            // Collect all tokens in same group for the picker
  ...
  // For non-color tokens, normalize and compare computed values
...
/** Normalize a length value for comparison (strip whitespace, lowercase). */
 ...
 * Converts various formats to a canonical rgba() string.
 */
