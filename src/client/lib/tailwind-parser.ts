...
export interface ParsedProperty {
  ...
  property: string; // e.g. "backgroundColor", "paddingX", "borderRadius"
  label: string; // human readable: "Background", "Padding X", "Border Radius"
  value: string; // the Tailwind value: "primary", "4", "md"
  fullClass: string; // the complete class: "bg-primary", "px-4", "rounded-md"
  prefix?: string; // responsive/state prefix: "sm:", "hover:", "dark:"
}
...
// --- Prefix handling ---
const PREFIXES = /^(sm|md|lg|xl|2xl|hover|focus|focus-visible|active|disabled|dark|group-hover|peer-hover|first|last|odd|even|placeholder|aria-invalid):/;
...
export function parseClasses(classString: string): StructuredProperties {
  ...
  for (const cls of classes) {
    ...
    for (const pattern of CLASS_PATTERNS) {
      ...
      if (match) {
        // Filter: text-{color} vs text-{size} disambiguation
        ...
        // Filter: border-{color} vs border-{width}
...
// --- Build the new class from a property change ---
...
// --- Available values for each property type ---
